...
@author: Jakob Häggström / Manal Rahal
...
Edited by Manal Rahal August 2022
...
def count_contact_scontact(seq, featvec, get_feat=False):
    ...
    if 'count' in featvec:  # This section counts the freq of each nucleotides.
        ...
        countdict = {'Fa': None, 'Ft': None, 'Fc': None, 'Fg': None, 'Length':None}
    ...
    if 'sulfurcount' in featvec:
        
        countsulfurdict={'Fsulf': None}

        Nsulf = seq.count('*')

        countsulf = np.array([Nsulf])
        
        if "-P=O" in seq:
            countsulf = countsulf - 1

        for i, key in enumerate(countsulfurdict.keys()):

            countsulfurdict[key] = countsulf[i]

        dictvec.append(countsulfurdict)
    ...
    if 'position' in featvec:  # This section counts the freq of each nucleotides.

        positiondict = {'First': None, 'Last': None}
        nucleotides = ['A', 'T', 'C', 'G', 'O']
        
        if seq[-1] == 'O':
            seq = seq[:-4]
    
        pos =  [ seq[i] for i in (0, -1) ]
        
        first_index = "".join(map(str, [i for i, j in enumerate(nucleotides) if j == pos[0]]))
        last_index = "".join(map(str,[i for i, j in enumerate(nucleotides) if j == pos[1]]))

        position = np.array([first_index, last_index])

        for i, key in enumerate(positiondict.keys()):

            positiondict[key] = position[i]

        dictvec.append(positiondict)
...
def encode_input(seq, featvec, temp=50):
    ...
    check = [(feat in ['count', 'contact', 'scontact','position','sulfurcount']) for feat in featvec]
...
if __name__ == "__main__":
    featvec = ['count', 'contact','scontact','position']
    l = ['ATGC', 'AAATTTATCTGA','TTCCGG','CGTTTTA']
